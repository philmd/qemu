...
#define LE_ADDR(vmin, vmax, imin, imax, address) \
            TESTADDR(0, vmin, vmax, imin, imax, address)
#define BE_ADDR(vmin, vmax, imin, imax, address) \
            TESTADDR(1, vmin, vmax, imin, imax, address)
...
static const char *cmdline_args = "-M none -device mmio-testdev";// -trace events=/tmp/events";
...
static void test_memadjust_readb(gconstpointer data)
{
    ...
    if (exp_dbg) fprintf(stderr, "@%08x exp_i:0x%08x\n", BE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset), exp_value);
    uint8_t x = qtest_readb(s, LE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset));
    if (exp_dbg) fprintf(stderr, "@%08x exp_o:0x%08x\n", BE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset), x);
...
static void test_memadjust_readw(gconstpointer data)
{
    ...
    uint16_t x = qtest_readw(s, LE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset));
...
static void test_memadjust_readl(gconstpointer data)
{
    ...
    uint32_t x = qtest_readl(s, LE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset));
...
static void test_memadjust_readq(gconstpointer data)
{
    ...
    uint64_t x = qtest_readq(s, LE_ADDR(a->vmin, a->vmax, a->imin, a->imax, a->offset));
...
static void add_memadjust_test_case(int endianness, int size, int vmin, int vmax, int imin, int imax, uint32_t offset, uint64_t value, void (*fn)(const void *))
{
    ...
    name = g_strdup_printf("memory-adjust/read%c/%ce/@%u/vmin%u/vmax%u/imin%u/imax%u",
        size_ch[size],
        endian_ch[endianness],
        offset,
        vmin, vmax,
        imin, imax);
    //fprintf(stderr, "%d %s\n", test_cnt++, name);
...
int main(int argc, char **argv)
{
    ...
    add_memadjust_test_cases(2, 0x04, 0x6677, test_memadjust_readw);
    ...
    add_memadjust_test_cases(8, 0x00, 0x0011223344556677, test_memadjust_readq); //XXX
