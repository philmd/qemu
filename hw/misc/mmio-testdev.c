...
#define TYPE_MMIO_TEST_DEV "mmio-testdev"
...
static uint64_t mmio_testdev_read_le(void *opaque, hwaddr offset, unsigned size)
{
    ...
    case 1:
        value = ldub_p(&s->buf.u8[offset]);
        fprintf(stderr, "dev_read_LE_%d @%ld: 0x%02lx\n", size, offset, value);
        break;
    case 2:
        value = lduw_le_p(&s->buf.u16[offset]);
        fprintf(stderr, "dev_read_LE_%d @%ld: 0x%04lx\n", size, offset, value);
        break;
    case 4:
        //value = ldl_le_p(&s->buf.u32[offset]);
        value = cpu_to_le32(s->buf.u32[offset]); // host cpu to BE
        fprintf(stderr, "dev_read_LE_%d @%ld: 0x%08lx\n", size, offset, value);
        break;
    case 8:
        value = ldq_le_p(&s->buf.u64[offset]);
        fprintf(stderr, "dev_read_LE_%d @%ld: 0x%016lx\n", size, offset, value);
        break;
    }
